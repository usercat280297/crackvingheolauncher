  ...
  useEffect(() => {
    const fetchGameDetails = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const gameData = {
            ...
            backgroundImage: steamData.backgroundImage || `http://localhost:3000/api/steam/image/${id}/library`,
            screenshots: steamData.screenshots || [],
            genres: typeof steamData.genres === 'string' ? steamData.genres.split(', ').filter(g => g.trim()) : [],
            rating: steamData.rating,
            size: steamData.size || '50 GB',
            systemRequirements: steamData.requirements || generateSystemRequirements(parseInt(id)),
            steamUrl: `https://store.steampowered.com/app/${id}/`,
            tags: typeof steamData.genres === 'string' ? steamData.genres.split(', ').filter(g => g.trim()) : [],
      ...
      const fallbackGame = {
        ...
        screenshots: [
          `https://cdn.akamai.steamstatic.com/steam/apps/${id}/ss_1.jpg`,
          `https://cdn.akamai.steamstatic.com/steam/apps/${id}/ss_2.jpg`,
          `https://cdn.akamai.steamstatic.com/steam/apps/${id}/ss_3.jpg`
        ],
        cover: `https://cdn.akamai.steamstatic.com/steam/apps/${id}/header.jpg`,
        backgroundImage: `https://cdn.akamai.steamstatic.com/steam/apps/${id}/library_600x900.jpg`,
        hasOnlineFix: parseInt(id) % 3 === 0,
        hasBypassCrack: parseInt(id) % 5 === 0,
        updateLog: generateUpdateLog(),
        reviews: generateReviews(parseInt(id)),
                          ...
                          {game.screenshots?.map((screenshot, index) => (
                            ...
                            >
                              <img 
                                ...
                                className="w-full h-full object-cover"
                                onError={(e) => {
                                  e.target.src = `http://localhost:3000/api/steam/image/${id}/header`;
                                }}
                              />
                            </button>
                          ))}
                          ...
                          {game.systemRequirements?.minimum ? (
                            typeof game.systemRequirements.minimum === 'string' ? (
                              <div dangerouslySetInnerHTML={{ 
                                __html: game.systemRequirements.minimum.replace(/<br\s*\/?>/gi, '<br/>').replace(/<strong>(.*?)<\/strong>/gi, '<div class="text-gray-400 text-sm mb-2">$1</div>').replace(/:\s*/g, '</div><div class="text-white text-base font-bold mb-4">') 
                              }} />
                            ) : (
                        ...
                        <div className="space-y-4">
                          {game.systemRequirements?.recommended ? (
                            typeof game.systemRequirements.recommended === 'string' ? (
                              <div dangerouslySetInnerHTML={{ 
                                __html: game.systemRequirements.recommended.replace(/<br\s*\/?>/gi, '<br/>').replace(/<strong>(.*?)<\/strong>/gi, '<div class="text-gray-400 text-sm mb-2">$1</div>').replace(/:\s*/g, '</div><div class="text-white text-base font-bold mb-4">') 
                              }} />
                            ) : (
